...
# tool0 → 5 cm → palm link (top) → 16 cm → finger base (actual grasp point)
# User measured ~28 cm tool0→fingertip; finger base (knuckle pivot) is ~16 cm below palm link.
GRIPPER_PALM_HEIGHT = 0.160  # m — robotiq_palm link to finger attachment point
...
GRASP_HEIGHT_OFFSET = 0.21   # m  ← TOOL0_TO_FINGER_BASE_Z (0.21m): finger base at object centre
...
class GraspExecutorNode(Node):
    ...
    def _execute_grasp_sequence(self, grasp_pose: PoseStamped):
        try:
            ...
            # In real hardware mode, always use calibrated gz regardless of GraspGen z.
            # GraspGen z from a partial/noisy point cloud is unreliable — it can be
            # anywhere from 0.3 to 0.8 m.  We know exactly where the finger base must
            # be: at obj_cz (object centre height).  Use XY from GraspGen, override Z.
            if self._real_hardware and obj_geom is not None:
                gz_calibrated = max(GRASP_Z_MIN, obj_cz + GRASP_HEIGHT_OFFSET)
                self.get_logger().info(
                    f"Real hardware: gz overridden {gz:.3f} → {gz_calibrated:.3f} "
