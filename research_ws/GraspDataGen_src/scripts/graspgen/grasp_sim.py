...
from warp_kernels import (
    ...
    get_joint_pos_kernel, set_is_success_kernel, get_body_pos_kernel, transform_inverse_kernel,
    transform_inverse_isaaclab_kernel, get_cspace_positions_kernel, get_bite_points_kernel
)
import time
...
class GraspingSimulation:
    ...
    def build_grasp_sim_scene_cfg(self, num_envs):
        ...
        class GraspingSceneCfg(InteractiveSceneCfg):
            ...
            gripper = ArticulationCfg(
                ...
                init_state=ArticulationCfg.InitialStateCfg(
                    joint_pos={
                        "palm_finger_b_joint": -0.18,
                        "palm_finger_c_joint": -0.17,
                        "finger_a_joint_1": 0.06,
                        "finger_b_joint_1": 0.06,
                        "finger_c_joint_1": 0.06,
                        "finger_a_joint_2": 0.01,
                        "finger_b_joint_2": 0.01,
                        "finger_c_joint_2": 0.01,
                        "finger_a_joint_3": -1.20,
                        "finger_b_joint_3": -1.20,
                        "finger_c_joint_3": -1.20,
                }),#joint_pos={"finger_joint": -0.62},),
            ...
            object = RigidObjectCfg(
                ...
                init_state=RigidObjectCfg.InitialStateCfg(pos=(0.0, 0.0, 100.0), lin_vel=(0.0, 0.0, 0.0), ang_vel=(0.0, 0.0, 0.0))
            )
            ...
            contact_forces2 = ContactSensorCfg(
                update_period=0.0, history_length=0, debug_vis=False
            )
        ...
        if len(finger_colliders_list) > 2:
            scene_cfg.contact_forces2.prim_path="{ENV_REGEX_NS}/Robot/" + str(finger_colliders_list[2])
        return scene_cfg
    ...
    def validate_grasps(self):
        ...
        print(f"DEBUG cspace_positions[0]: {self.cspace_positions.numpy()[0].tolist()}")
        print(f"DEBUG cspace_positions shape: {self.cspace_positions.numpy().shape}")
        # DEBUG: print first 3 raw transforms
        import warp as _wp
        _raw = grasp_sim_buffer.transforms.numpy()
        print(f'DEBUG TRANSFORMS shape: {_raw.shape}')
        for i in range(min(3, len(_raw))):
            print(f'DEBUG transform[{i}]: pos={_raw[i,:3]}, quat={_raw[i,3:]}')
    ...
    def run_grasp_sim(self, start_idx, num_envs, buff, batch_count=None, total_batches=None):
        ...
        with build_simulation_context(device=self.config.device, gravity_enabled = False, auto_add_lighting=True, sim_cfg=sim_cfg) as sim:
            ...
            contact_forces2 = scene["contact_forces2"] if "contact_forces2" in scene.keys() else None
            ...
            _mass = float(object.data.default_mass[0])
            if not _math.isfinite(_mass) or _mass <= 0:
                _mass = 0.1  # default 100g if mass uninitialized
            force_magnitude = Gs * 9.81 * _mass
            ...
            while simulation_app.is_running():
                ...
                if count == 0:
                    ...
                    print(f"DEBUG COUNT0 pregrasp_joint_pos: {joint_pos[0].tolist()}")
                    print(f"DEBUG COUNT0 pregrasp_cspace[0]: {buff.pregrasp_cspace_positions.numpy()[start_idx].tolist()}")
                    ...
                    # Set object poses by inverting pregrasp transforms (pure numpy, no Warp)
                    root_state = object.data.default_root_state.clone()
                    xforms = buff.pregrasp_transforms.numpy()  # [total, 7] warp fmt: x,y,z,qx,qy,qz,qw
                    for _i in range(num_envs):
                        _idx = start_idx + _i
                        _px, _py, _pz = float(xforms[_idx,0]), float(xforms[_idx,1]), float(xforms[_idx,2])
                        _qx, _qy, _qz, _qw = float(xforms[_idx,3]), float(xforms[_idx,4]), float(xforms[_idx,5]), float(xforms[_idx,6])
                        # conjugate quaternion (= inverse for unit quat)
                        _cx, _cy, _cz, _cw = -_qx, -_qy, -_qz, _qw
                        # rotate pos by conjugate: v'= p + 2w(u x p) + 2(u x (u x p))
                        _t0 = _cy*_pz - _cz*_py;  _t1 = _cz*_px - _cx*_pz;  _t2 = _cx*_py - _cy*_px
                        _s0 = _cy*_t2 - _cz*_t1;   _s1 = _cz*_t0 - _cx*_t2;   _s2 = _cx*_t1 - _cy*_t0
                        _rx = _px + 2.0*(_cw*_t0 + _s0)
                        _ry = _py + 2.0*(_cw*_t1 + _s1)
                        _rz = _pz + 2.0*(_cw*_t2 + _s2)
                        # inverse position = -rotated, isaac lab quat order: qw,qx,qy,qz
                        root_state[_i, 0] = -_rx;  root_state[_i, 1] = -_ry;  root_state[_i, 2] = -_rz
                        root_state[_i, 3] = _cw;   root_state[_i, 4] = _cx;   root_state[_i, 5] = _cy;  root_state[_i, 6] = _cz
                        if _i == 0:
                            print(f"DEBUG NUMPY_INV: input pos=[{_px:.4f},{_py:.4f},{_pz:.4f}] quat=[{_qx:.4f},{_qy:.4f},{_qz:.4f},{_qw:.4f}]")
                            print(f"DEBUG NUMPY_INV: output pos=[{-_rx:.4f},{-_ry:.4f},{-_rz:.4f}]")
                    if not USE_ORIGIN_PLACEMENT:
                        root_state[:, :3] += scene.env_origins
                    print(f"DEBUG BEFORE_WRITE: root_state[0,:7]={root_state[0,:7].tolist()}")
                    object.write_root_pose_to_sim(root_state[:, :7])
                    object.write_root_velocity_to_sim(root_state[:, 7:])
                    scene.update(sim_dt)
                    print(f"DEBUG AFTER_WRITE: object_pos={object.data.root_pos_w[0].tolist()}")
                    ...
                    scene.reset()
                    scene.update(sim_dt)
                    print(f"DEBUG AFTER_RESET: object_pos={object.data.root_pos_w[0].tolist()}")
                    # RE-WRITE object pose after reset in case reset clobbered it
                    object.write_root_pose_to_sim(root_state[:, :7])
                    object.write_root_velocity_to_sim(root_state[:, 7:])
                    scene.update(sim_dt)
                    print(f"DEBUG AFTER_REWRITE: object_pos={object.data.root_pos_w[0].tolist()}")
                    # Comprehensive diagnostic
                    print(f"DEBUG body_names: {gripper.body_names}")
                    print(f"DEBUG link_paths[0]: {gripper.root_physx_view.link_paths[0]}")
                    print(f"DEBUG contact0_prim: {scene_cfg.contact_forces0.prim_path}")
                    print(f"DEBUG contact1_prim: {scene_cfg.contact_forces1.prim_path}")
                    for bi, bn in enumerate(gripper.body_names):
                        bp = gripper.data.body_pos_w[0, bi].tolist()
                        print(f"DEBUG body[{bi}] {bn}: pos={bp}")
                    obj_p = object.data.root_pos_w[0].tolist()
                    print(f"DEBUG object_at_start: {obj_p}")
                ...
                if not data_done and sim_time >= force_start_time and sim_time < force_end_time and not self.config.disable_sim:
                    ...
                    # Apply force directly in world frame (is_global=True by default)
                    object.set_external_force_and_torque(world_forces[current_sequence], zero_torque)
                    ...
                    # DEBUG
                    print(f"DEBUG object_pos: {object.data.root_pos_w[0].tolist()}")
                    print(f"DEBUG gripper_pos: {gripper.data.root_pos_w[0].tolist()}")
                    print(f"DEBUG joint_pos: {gripper.data.joint_pos[0].tolist()}")
                    print(f"DEBUG soft_limits_closed: {gripper.data.soft_joint_pos_limits[0,:,grasp_mode].tolist()}")
                    print(f"DEBUG contact_forces0: {cdata0.net_forces_w[0].tolist()}")
                    print(f"DEBUG contact_forces1: {cdata1.net_forces_w[0].tolist()}")
                    print(f"DEBUG object_mass: {object.data.default_mass[0].item()}")
                    ...
                    if contact_forces2 is not None:
                        cdata2 = contact_forces2.data
                        non_zero_forces2 = torch.all(torch.norm(cdata2.net_forces_w, dim=-1) > 0, dim=1)
                        # Success = any 2 of 3 fingers have contact
                        non_zero_forces = ((non_zero_forces0.int() + non_zero_forces1.int() + non_zero_forces2.int()) >= 2).bool()
                    else:
                        # Fallback for 2-finger grippers
