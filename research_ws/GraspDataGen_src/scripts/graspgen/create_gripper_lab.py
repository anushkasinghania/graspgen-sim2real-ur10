...
import warp as wp
#wp.init()
#wp.config.enable_mempool = False
...
class GripperCreator:
    ...
    def build_scene_cfg(self):
        ...
        class GripperCreatorSceneCfg(InteractiveSceneCfg):
            ...
            # gripper-> I modified and added pos & spawn
            gripper = ArticulationCfg(
                prim_path="{ENV_REGEX_NS}/Robot",
                init_state=ArticulationCfg.InitialStateCfg(
                    joint_pos={"finger_a_joint_1": 0.1, "finger_b_joint_1": 0.1, "finger_c_joint_1": 0.1, "finger_a_joint_3": -0.1, "finger_b_joint_3": -0.1, "finger_c_joint_3": -0.1,}),
                actuators={
    ...
    def save_scene_full(self, folder_name, gripper):
        ...
        if num_bodies == 0:
            raise RuntimeError("No bodies detected in gripper USD. Check USD articulation structure.")
        gripper_bodies = {}
        ...
        for b_idx, name in enumerate(gripper.body_names):
            ...
            _cm, _lti = usd_tools.get_prim_collision_mesh(
                prim=self.scene.stage.GetPrimAtPath(prim_path), collision_enabled=True, device=self.config.device)
        ...
        print("body_pos_w shape =", gripper.data.body_pos_w.shape)
        print("body_quat_w shape =", gripper.data.body_quat_w.shape)
        print("================================\n")
            
       # here added 2 more lines above it
        pos_tensor = gripper.data.body_pos_w.to(self.config.device).contiguous()
        quat_tensor = gripper.data.body_quat_w.to(self.config.device).contiguous()
        
        
        print("RAW pos tensor shape:", pos_tensor.shape)
        print("RAW quat tensor shape:", quat_tensor.shape)

        # reshape to warp expected structure
        pos_tensor = pos_tensor.view(self.num_envs, num_bodies, 3)
        quat_tensor = quat_tensor.view(self.num_envs, num_bodies, 4)
        
        print("RESHAPED pos tensor shape:", pos_tensor.shape)
        print("RESHAPED quat tensor shape:", quat_tensor.shape)
        
        pos_wp = wp.from_torch(pos_tensor, dtype=wp.vec3)
        quat_wp = wp.from_torch(quat_tensor, dtype=wp.vec4)

        #pos_wp = wp.from_torch(pos_tensor, dtype=wp.vec3, device=self.config.device)
        #quat_wp = wp.from_torch(quat_tensor, dtype=wp.vec4, device=self.config.device)

        print("\n===== DEBUG: Kernel Input =====")
        print("num_envs:", self.num_envs)
        print("num_bodies:", len(gripper.body_names))
        print("pos shape:", pos_tensor.shape)
        print("quat shape:", quat_tensor.shape)
        print("device:", self.config.device)
        print("body names:", gripper.body_names)
        print("================================\n")
        
        wp.launch(get_transforms_kernel,
                dim=(num_bodies, self.num_envs),
                inputs=[pos_wp, quat_wp, body_transforms, self.num_envs, reverse_order, local_transform_inverses],
    ...
    def create_gripper(self, save_gripper=False, measure_convergence=DEFAULT_MEASURE_CONVERGENCE, convergence_iterations=DEFAULT_CONVERGENCE_ITERATIONS):
        ...
        sim_cfg = sim_utils.SimulationCfg(
            device="cpu", # this changed
            dt=dt,  # Set physics timestep based on FPS
            physx=sim_utils.PhysxCfg()
        )
        with build_simulation_context(device="cpu", gravity_enabled = False, auto_add_lighting=True, sim_cfg=sim_cfg) as sim:
            sim._app_control_on_stop_handle = None
            self.config.device = "cpu" #added this, it forces all wrap kernels to use system RAM istead of GPU
            ...
            # here also added 2 more lines above it
            ...
            print("DEBUG: about to construct Gripper object")

            return_gripper = Gripper(self.config, gripper_data_out)
            print(f"Gripper Created from {self.config.gripper_file}")
            
            print("Total bodies:", len(gripper.body_names))
            print("Body names:", gripper.body_names)
            print("Body pos tensor shape:", gripper.data.body_pos_w.shape)
            print("Body quat tensor shape:", gripper.data.body_quat_w.shape)
            
            wp.synchronize()
            wp.clear_kernel_cache()
        
        if hasattr(return_gripper, "bite_points"):
            return_gripper.bite_points = return_gripper.bite_points.to("cpu")
