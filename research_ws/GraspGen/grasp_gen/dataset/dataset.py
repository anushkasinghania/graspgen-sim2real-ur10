    ...
    def __getitem__(self, idx):
        ...
        if self.load_discriminator_dataset:
            ...
            if not load_contact_batch:
                ...
                grasps_val = outputs["grasps"]
                if isinstance(grasps_val, list):
                    grasps_val = grasps_val[0]
                if isinstance(grasps_val, torch.Tensor):
                    outputs["grasps"] = grasps_val.float()
                else:
                    outputs["grasps"] = torch.from_numpy(
                        np.array(grasps_val).astype(np.float32)
                    )
...
def load_discriminator_batch_with_stratified_sampling(
    ...
    if include_true_pos:
        ...
        # Filter out NaN rows from positive grasps
        valid_mask = ~np.isnan(grasps_positive).any(axis=(1, 2))
        grasps_positive = grasps_positive[valid_mask]
        if len(grasps_positive) == 0:
            include_true_pos = False
    if not include_true_pos:
        num_neg_true_grasps += num_pos_true_grasps
        num_pos_true_grasps = 0
        num_pos_true_onpolicy_grasps = 0

    if grasps_negative is not None:
        # Filter out NaN rows from negative grasps
        valid_mask = ~np.isnan(grasps_negative).any(axis=(1, 2))
        grasps_negative = grasps_negative[valid_mask]
        if len(grasps_negative) == 0:
            grasps_negative = None
