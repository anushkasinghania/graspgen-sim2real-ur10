...
def render_point_cloud_from_object(
    ...
    num_points: int,
    prob_object_only: float = 1.0,
) -> Tuple[Union[np.array, None], DataLoaderError]:
    # prob_object_only=1.0: always use object-only pyrender path.
    # The with-table branch uses scene_synthesizer (python-fcl) which is not
    # installable on ARM64/Jetson.  Object-only rendering uses pyrender with
    # random camera viewpoints — this produces single-viewpoint partial PCs
    # that match real OAK-D captures after table-removal preprocessing.
