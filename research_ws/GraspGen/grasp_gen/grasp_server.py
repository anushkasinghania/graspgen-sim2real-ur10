...
def load_grasp_cfg(gripper_config: str) -> omegaconf.DictConfig:
    ...
    if "eval" in cfg and "checkpoint" in cfg.eval and cfg.eval.checkpoint:
        cfg.eval.checkpoint = str(ckpt_root_dir / cfg.eval.checkpoint)

    if "discriminator" in cfg and "checkpoint" in cfg.discriminator and cfg.discriminator.checkpoint:
...
class GraspGenSampler:
    ...
    @staticmethod
    def run_inference(
        object_pc:Union[np.ndarray, torch.Tensor],
